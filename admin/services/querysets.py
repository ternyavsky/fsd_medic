...
def user_profile_data(pk):
    ...
    process_notes = notes.filter(
        user_id__in=Subquery(users.values("id")), status="In processing"
    )
    miss_notes = notes.filter(
        user_id__in=Subquery(users.values("id")), time_start__gt=datetime.now()
    )
    result = {
        "user": users,
...
def country_profile_data(name):
    ...
    cities = cities.annotate(
        ...
        quant_centers_today=Count(
            "center", distinct=True, filter=Q(center__created_at__date=date.today())
        ),
        quant_clinics=Count("clinic", distinct=True),
        quant_clinics_today=Count(
            "clinic", distinct=True, filter=Q(clinic__created_at__date=date.today())
        ),
        quant_users=Count("user", distinct=True),
        quant_users_today=Count(
            "user", distinct=True, filter=Q(user__created_at__date=date.today())
        ),
    )
    cities = cities.order_by(
        -(F("quant_centers") + F("quant_clinics") + F("quant_users"))
    )
    ...
    result = {"cities": cities, "country": countries}
    return result
...
def center_profile_data(pk):
    ...
    obj = {"center": centers, "pacients": result}
    return obj
...
def main_page_data():
    ...
    group_1 = users.filter(
        birthday__year__lte=date.today().year - 10,
        birthday__year__gte=date.today().year - 20,
    )
    group1_mans = group_1.filter(sex="Мужчина").count()
    group1_womans = group_1.filter(sex="Женщина").count()

    group_2 = users.filter(
        birthday__year__lte=date.today().year - 20,
        birthday__year__gte=date.today().year - 30,
    )
    group2_mans = group_2.filter(sex="Мужчина").count()
    group2_womans = group_2.filter(sex="Женщина").count()

    group_3 = users.filter(
        birthday__year__lte=date.today().year - 30,
        birthday__year__gte=date.today().year - 40,
    )
    group3_mans = group_3.filter(sex="Мужчина").count()
    group3_womans = group_3.filter(sex="Женщина").count()

    group_4 = users.filter(
        birthday__year__lte=date.today().year - 40,
        birthday__year__gte=date.today().year - 50,
    )
    group4_mans = group_4.filter(sex="Мужчина").count()
    group4_womans = group_4.filter(sex="Женщина").count()

    group_5 = users.filter(
        birthday__year__lte=date.today().year - 50,
        birthday__year__gte=date.today().year - 60,
    )
    group5_mans = group_5.filter(sex="Мужчина").count()
    group5_womans = group_5.filter(sex="Женщина").count()

    group_6 = users.filter(
        birthday__year__lte=date.today().year - 60,
        birthday__year__gte=date.today().year - 70,
    )
    ...
    diseases = diseases.annotate(most_count=Count("user"))
