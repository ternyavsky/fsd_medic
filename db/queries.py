...
def get_notes(**kwargs):
    ...
    return (Note.objects.filter(**kwargs)
            .select_related("doctor", "doctor__group", "doctor__country", "doctor__main_center",
                            "doctor__main_center__country", "user__country", "center", "user", "user__group",
                            "user__main_center",
    ...
    if messages is None:
        messages = (Message.objects.filter(**kwargs)
                    .select_related("chat", "chat__to_user", "chat__to_user__country", "chat__to_user__main_center",
                                    "chat__to_center", "chat__to_user__main_center__country", "chat__from_user",
                                    "chat__from_user__country",
                                    "chat__to_user__group",
                                    "chat__from_user__main_center", "chat__from_user__main_center__country",
                                    "chat__from_user__group",
                                    "chat__from_user__country",

                                    "chat__from_center", "chat__from_center__country", "news", "news__center",
                                    "news__disease", "news__center",
                                    "news__center__country", "user", "user__group", "user__main_center",
                                    "user__country",
...
def get_chats(**kwargs):
    return Chat.objects.filter(**kwargs).select_related('to_user', 'to_center', 'from_center', 'from_user')
