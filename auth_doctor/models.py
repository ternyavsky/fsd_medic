...
class Doctor(AbstractBaseUser, BaseModel):
    ...
    email = models.CharField(
        verbose_name=_("Почта"), max_length=220, null=True, blank=True
    )
    ...
    clinic = models.ForeignKey(
        ...
        null=True,
        on_delete=models.CASCADE,
        verbose_name=_("Клиника, где врач работает"),
        blank=True,
    )
...
class LinkToInterview(BaseModel):
    link = models.CharField(
        verbose_name=_("Ссылка на интервью"), max_length=220, unique=True
    )
...
class Interview(BaseModel):
    doctor = models.ForeignKey(
        Doctor, on_delete=models.CASCADE, null=True, verbose_name=_("Врач")
    )
    clinic = models.ForeignKey(
        Clinic, on_delete=models.CASCADE, null=True, verbose_name=_("Клиника")
    )
    center = models.ForeignKey(
        Center, on_delete=models.CASCADE, null=True, verbose_name=_("Центр")
    )
