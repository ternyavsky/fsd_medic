...
def create_message(
    instance: User | Doctor, chat: Chat, text: str, reply: Message = None
):
    print(type(instance) == User)
    if type(instance) == User:
        user = cache.get_or_set("users", get_users()).filter(id=instance["id"]).first()
        message = Message.objects.create(
            user=user, text=text, chat=chat, reply=reply if reply else None
        )
        message.save()
    else:
        doctor = (
            cache.get_or_set("doctors", get_doctors()).filter(id=instance["id"]).first()
        )
        message = Message.objects.create(
            doctor=doctor, text=text, chat=chat, reply=reply if reply else None
        )
        message.save()
